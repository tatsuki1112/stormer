"""Tests for Tavily health checker."""
...
class TestTavilyHealthChecker:
    """Test suite for TavilyHealthChecker."""

    @pytest.fixture
    def checker(self):
        """Create a TavilyHealthChecker instance for testing."""
    ...
    def test_initialization(self, checker):
        """Test that TavilyHealthChecker initializes correctly."""
    ...
    async def test_successful_health_check(self, checker):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            assert "has_answer" in result.details

    @pytest.mark.asyncio
    async def test_successful_health_check_without_answer(self, checker):
        """Test successful health check with response but no answer field."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            assert result.status == ServiceStatus.HEALTHY
            assert result.details is not None
            assert result.details.get("has_answer") is False
    ...
    async def test_timeout_error(self, checker):
        ...
        mock_client_instance = AsyncMock()
        mock_client_instance.post.side_effect = TimeoutException("Request timed out")
    ...
    @pytest.mark.asyncio
    async def test_post_request_params(self, checker):
        """Test that POST request is made with correct parameters."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"answer": "test"}
        mock_response.headers = {}
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            assert call_args[1]["json"]["max_results"] == 1
